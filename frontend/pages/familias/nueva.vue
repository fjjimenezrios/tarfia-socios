...
const form = reactive({
  apellidos: "", nombre_tutor1: "", nombre_tutor2: "",
  email: "", telefono: "", telefono2: "",
  direccion: "", localidad: "", cp: "", notas: "",
})
...
async function guardar() {
  ...
  saving.value = true
  try {
    await apiFetch("/familias/", { method: "POST", body: form })
    toast.add({ title: "Familia creada", color: "green", icon: "i-heroicons-check-circle" })
...
<template>
  <div class="max-w-2xl space-y-6">
    <div class="flex items-center gap-3">
      ...
      <h1 class="text-xl font-bold text-gray-900 dark:text-white">Nueva familia</h1>
    </div>
    ...
    <div class="flex justify-end gap-3">
      <UButton to="/familias" variant="outline" color="gray">Cancelar</UButton>
      <UButton color="amber" :loading="saving" icon="i-heroicons-check" @click="guardar">Guardar familia</UButton>
    </div>
  </div>
</template>
